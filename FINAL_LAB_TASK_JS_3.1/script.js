...
function fdeg()
{
    let form=document.querySelector('#form-deg');

    let inf=document.querySelector('.info');

    let c=0;
    console.log(form.deg);
    form.addEventListener('submit',(e)=>
    {
        e.preventDefault();
        for(let i=0;i<form.deg.length;i++)
        {
            if(form.deg[i].checked==true)
            {
                c=c+1;
            }
        }
        ...
        if(c>0)
        {
            inf.innerHTML="Your degrees are<br>";
            for(let i=0;i<form.deg.length;i++)
            {
                if(form.deg[i].checked==true)
                    inf.innerHTML+=form.deg[i].value+'<br>';
            }
        }
        else
        {
            inf.innerHTML="please chose atleast one degree ";
        }
    })
